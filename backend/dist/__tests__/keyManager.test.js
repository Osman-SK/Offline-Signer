...
describe('keyManager', () => {
    ...
    describe('importKeypair', () => {
        it('should import keypair from base58 format', () => {
            const result = keyManager.importKeypair(TEST_KEYPAIR_NAME, TEST_PRIVATE_KEY_BASE58, 'base58');
            expect(result).toBeDefined();
            expect(result.name).toBe(TEST_KEYPAIR_NAME);
        ...
        it('should import keypair from base64 format', () => {
            ...
            const result = keyManager.importKeypair(TEST_KEYPAIR_NAME, secretKey, 'base64');
            expect(result).toBeDefined();
            expect(result.name).toBe(TEST_KEYPAIR_NAME);
        ...
        it('should import keypair from JSON format', () => {
            ...
            const result = keyManager.importKeypair(TEST_KEYPAIR_NAME, secretKey, 'json');
            expect(result).toBeDefined();
            expect(result.name).toBe(TEST_KEYPAIR_NAME);
            expect(result.publicKey).toBe(keypair.publicKey.toBase58());
        });
        it('should default to base58 format', () => {
            const result = keyManager.importKeypair(TEST_KEYPAIR_NAME, TEST_PRIVATE_KEY_BASE58);
            expect(result.publicKey).toBe(EXPECTED_PUBLIC_KEY);
        });
        it('should throw error if name is missing', () => {
            expect(() => {
        ...
        it('should throw error for unknown format', () => {
            expect(() => {
                keyManager.importKeypair(TEST_KEYPAIR_NAME, TEST_PRIVATE_KEY_BASE58, 'unknown');
